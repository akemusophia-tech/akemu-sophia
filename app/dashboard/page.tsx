...
function LoadingRows() { return <>{[1, 2, 3].map((item) => <tr key={item}><td colSpan={5} className="px-5 py-5"><div className="h-10 animate-pulse rounded-lg bg-muted" /></td></tr>)}</> }

function TicketDetail({ ticket, close, updateStatus, updating }: { ticket: Ticket; close: () => void; updateStatus: (status: string) => void; updating: boolean }) {
  const [messages, setMessages] = useState<Message[]>([{ role: 'customer', content: ticket.description }])
